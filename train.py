...
def main():
    ...
    for epoch in range(start_epoch, opt.num_epochs):
        ...
        # train for one epoch
        best_rsum, best_r1, best_rsum1 = train(opt, train_loader, model, epoch, val_loader, best_rsum, best_rsum1, best_r1, train_loader_length)
        # evaluate on validation set
        rsum, r1, rsum_all = validate(opt, val_loader, model)
        ...
        is_best = rsum_all > best_rsum1
        best_rsum1 = max(rsum_all, best_rsum1)
        best_r1 = max(r1, best_r1)
        save_checkpoint1({
            'epoch': epoch + 1,
            'model': model.state_dict(),
...
def train(opt, train_loader, model, epoch, val_loader, best_rsum, best_rsum1, best_r1, train_loader_length):
    ...
    return best_rsum, best_r1, best_rsum1
...
def validate(opt, val_loader, model):
    ...
    return currscore, r1, currscore + r10 + r10i

def save_checkpoint1(state, is_best, filename='checkpoint.pth_all.tar', prefix=''):
    torch.save(state, prefix + filename)
    if is_best:
        print('save best')
        shutil.copyfile(prefix + filename, prefix + 'model_best.pth_all.tar')
